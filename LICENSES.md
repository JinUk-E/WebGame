# 에셋 출처·라이선스 기록

NHN NAN 2026 사전과제 규정에 따른 AI 생성·외부 에셋 기록.

## AI 생성 오디오 (2026-08-04)

전부 로컬 도구로 생성 — 외부 유료 서비스·저작권 소스 미사용.

| 파일 | 생성 방법 |
|------|-----------|
| `Audio/Voice/fake1_*.wav` `fake2_*.wav` | Windows TTS(Microsoft Heami, ko-KR) 합성 → ffmpeg 가공 (피치 다운 ~4반음, 템포 보정, 에코). 뭉갬 벌은 로우패스 580Hz — "할아버지 목소리를 흉내 내는 존재" 연출 |
| `Audio/Voice/popopo_*.wav` | 동일 TTS → 피치 다운 6반음 + 에코 + 로우패스 (속삭임) |
| `Audio/Voice/true_signal_*.wav` | TTS 곡소리("아이고…")·염불("나무아미타불 관세음보살") 2트랙 → ffmpeg 믹스 + 3탭 에코 |
| `Audio/Voice/rescue_*.wav` | 동일 TTS → 피치 다운 (K씨 남성 톤, 에코 없음 — 진짜 사람) |
| `Audio/Voice/tv_hint.wav` `urge.wav` | 동일 TTS 근사 원음 (소년 독백) |
| `Audio/SFX_Heartbeat/heartbeat_loop.wav` | 절차 합성 (Python — 저주파 감쇠 사인 lub-dub 루프) |
| `Audio/SFX_Window/window_knock.wav` `window_rattle.wav` | 절차 합성 (Python — 감쇠 사인 + 노이즈 어택 / 진폭 변조 노이즈) |
| `Art/UI/heart128.png` | 절차 생성 (에디터 스크립트 — 임플리시트 하트 곡선) |
| `Art/Smoke/white32.png` | 절차 생성 (에디터 스크립트 — 단색) |

## 추가 생성분 (2026-08-06 — 명세 v0.5)

| 파일 | 생성 방법 |
|------|-----------|
| `Audio/SFX_Corner/whisper_loop.wav` | 절차 합성 (Python 표준 라이브러리, `Tools/gen_whisper_loop.py` 시드 20260806 — 대역제한 노이즈 300~1600Hz + 저주파 진폭 변조 + 루프 이음새 크로스페이드). 귀퉁이 속삭임 4채널 공용 루프 |
| `Art/Props/prop_silhouette.png` | 절차 생성 (Pillow, `Tools/art-gen/gen_silhouette.py` 시드 42500 — 무채색 탑뷰 형체 + 가우시안 블러). 어둠 속 실루엣 |

## 추가 생성분 (2026-08-07 — v0.6.1)

| 파일 | 생성 방법 |
|------|-----------|
| `Art/Props/prop_stand_marker.png` | 절차 생성 (Pillow, `Tools/art-gen/gen_marker.py` 시드 20260807 — 눌린 타원 원반 + 대각 브래킷 4개 + 먼지 알갱이, 4배 슈퍼샘플). 프롤로그 학습 구간 목적지 서클 |

## 추가 생성분 (2026-08-07 — 전화벨·흔들림)

| 파일 | 생성 방법 |
|------|-----------|
| `Audio/SFX_Phone/phone_ring.wav` | 절차 합성 (Python 표준 라이브러리, `Tools/gen_assault_sfx.py` 시드 20260807). 1970~80년대 시골 유선전화 벨 — 두 톤 종(605/742Hz, 비조화 부분음 ×1/2.72/5.36) 교대 타격 19.5Hz 트릴 + 하우징 공명 172/244Hz, 울림 1.0s ↔ 쉼 1.05s 2회, 복도 반사 3탭(37/71/113ms), 저역통과 1250Hz 3단 = "다른 방에서 울리는 소리". 22050Hz 모노 3.60s |
| `Audio/SFX_Handle/handle_rattle.wav` | 절차 합성 (같은 스크립트, 시드 20260808). 문 손잡이 덜컹 3타 ×2회(0/0.115/0.230, 0.800/0.915/1.030s) — 금속 걸쇠 공진 2150/3320Hz + 나무 문짝 96/148Hz + 문틀 340Hz, 저역통과 2800Hz 2단. 22050Hz 모노 1.35s. **타격 시각표는 C# `RattlePattern.DoorHandle`과 공유** (문짝 흔들림이 같은 표를 쓴다) |

## 추가 생성분 (2026-08-07 — v0.7 여명 체감)

| 파일 | 생성 방법 |
|------|-----------|
| `Art/Props/prop_dawn_patch_grid.png` | 절차 생성 (Pillow, `Tools/art-gen/gen_dawn_patch.py` 시드 20260809 — 사다리꼴 발자국(윗변 148px→아랫변 196px) + 중앙 세로살 그림자 1개 + 먼 쪽 감쇠(1−t)^1.35 + 미세 얼룩 260점, 3배 슈퍼샘플). 바닥 창틀 빛 무늬의 **선명한 격자 겹** 200×260px @PPU100 |
| `Art/Props/prop_dawn_patch_haze.png` | 같은 스크립트·같은 발자국에 가우시안 블러 27px. **형태 없는 빛무리 겹** — 격자와 알파를 반대로 움직여 "아침이 갈수록 선명해진다"를 만든다 (스프라이트에 블러를 걸 수 없어 두 겹으로 나눴다) |

**살 배치의 원본은 배포되는 창 아트 파일** `Art/Room/room_window.png`(136×83, 창 칸 알파 0 + 중앙 세로살 1개)다 — 창 아트를 다시 뽑아 살이 바뀌면 이 두 장도 함께 다시 뽑을 것.

## 외부 에셋

| 에셋 | 출처 | 라이선스 |
|------|------|----------|
| Pretendard 폰트 | [github.com/orioncactus/pretendard](https://github.com/orioncactus/pretendard) | SIL OFL 1.1 |
| **음향 효과·BGM** — `Audio/BGM_01_Main`, `BGM_02_Intro`, `BGM_03_Night`, `BGM_04_Ending`, `SFX_Clock`, `SFX_Door`(원본 m4a → ffmpeg mp3 변환), `SFX_Fear` | **Pixabay** (https://pixabay.com) | Pixabay Content License — 상업적 이용 무료, 출처 표기 불요(기록 목적으로 기재) |

**음성(`Audio/Voice/*`)은 Pixabay 수급분이 아니며 전량 자체 생성물이다** — 위 "AI 생성 오디오" 표 참조. 마찬가지로 `SFX_Heartbeat`·`SFX_Window`는 Python 절차 합성물이다.

AI 코딩: 개발 전 과정에 Claude Code(Anthropic) 사용 — AI 활용 기술 문서에 상세 기록 예정.
